...
- **Support for Amazon Simple Queuing Service rather than IMAP/POP3/SMTP. There are also scripts to send duplicati results to an AWS queue.**
...
| **legacy-sqs-fork** | \<None\>        | This is the original fork off of dupReport to support SQS. With the updates a couple years ago, dupReport was refactored so this fork is actually simpler than the old one. |

Since the original program is released under an MIT license, these edits are too. Please see the LICENSE file for more details.
